...
with aba2:
    ...
    vl4 = st.number_input('Digite aqui o salário do comprador de EPI a sua empresa.')
    st.text('Quantas horas mensais são gastas com compras de EPI? Ex: A média mensal é 40 horas.')
    vl5 = st.number_input('Digite aqui a quantidade de horas gastas com compras de EPI por mês na sua empresa.')
    st.text('Qual o salário do Técnico de Segurança na sua empresa? Ex: A média no mercado é de R$4.198,00.')
    vl6 = st.number_input('Digite aqui o salário do Técnico de Segurança.')
    ...
    vl7 = st.number_input('Digite quantas horas o Técnico de Segurança demanda para a compra de EPI.')
    st.text('Qual o tamanho do estoque atual em m²?')
    vl8 = st.number_input('Digite aqui a área do estoque:')
    st.text('Qual a porcentagem do estoque dedicado a EPI?')
    vl9 = st.number_input('A média no mercado é de 5% a 15%.')
    st.text('Qual o custo mensal do m² do estoque?')
    vl10 = st.number_input('A média no mercado é de R$20 a R$40')
    st.text('Qual o custo de movimentação de estoque?')
    vl11 = st.number_input('A média no mercado é de R$3,50.')
    st.text('Quantos EPIs são entregues por dia?')
    vl12 = st.number_input('Digite o número de EPIs entregues diariamente.')
    st.text('Quanto se perde de EPI por ano devido à deterioração?')
    vl13 = st.number_input('A média no mercado é de 1% a 5% por ano.')
    st.text('Qual o gasto anual com compras de EPI?')
    vl14 = st.number_input('Digite o valor de compras de EPI anual.')
